...
export default function Footer({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  return (
    <footer data-theme="dark" className="relative overflow-hidden bg-ink text-cream">
      <div className="container-bl pb-10 pt-[clamp(4rem,10vh,7rem)]">
        <div className="grid-bl gap-y-14">
          ...
          <div className="col-span-12 lg:col-span-5">
            ...
            <p className="display mt-8 text-h3 max-w-[16ch]">{dict.footer.claim}</p>

            <div className="mt-10 max-w-sm">
              <h2 className="eyebrow opacity-50">{dict.footer.newsletterTitle}</h2>
              <p className="mt-3 text-micro opacity-60">{dict.footer.newsletterBody}</p>
              <NewsletterForm
          ...
          <nav className="col-span-6 md:col-span-4 lg:col-span-2 lg:col-start-7">
        ...
        {/* Barra inferior */}
        <div className="mt-16 flex flex-col gap-4 border-t border-cream/15 pt-6 text-micro opacity-50 sm:flex-row sm:items-center sm:justify-between">
      ...
      >
        <span
          className="block whitespace-nowrap font-[family-name:var(--font-poppins)] font-medium leading-[0.78] text-cream/[0.07]"
          style={{ fontSize: "clamp(4rem, 21vw, 20rem)", letterSpacing: "-0.04em" }}
        >
          bitlogic
        </span>
      </div>
